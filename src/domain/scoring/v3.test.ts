...
describe('moteur v3 — les anciens chapitres ne bougent pas', () => {
  ...
  it('reste rejouable une fois dépassé par le v4', () => {
    // Ce test disait « les nouveaux chapitres s'ouvrent en v3 ». Il est devenu
    // faux le jour où le v4 est arrivé, et c'est très bien : ce qui doit tenir
    // dans la durée n'est pas qu'une version soit la plus récente, mais
    // qu'elle reste **jouable** quand elle ne l'est plus. Un chapitre publié
    // en v3 doit pouvoir se recalculer à l'identique (§78).
    expect(getScoringEngine(SCORING_VERSION).version).toBe(SCORING_VERSION);
