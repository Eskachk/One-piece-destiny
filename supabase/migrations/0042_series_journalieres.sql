-- Séries journalières pour le poste de commandement.
--
-- Même fonction que 0041, avec une clé `series` de plus : les trente
-- derniers jours, un objet par jour — inscriptions, ventes au marché (nombre
-- et Berries), achats en boutique (nombre et centimes). Les jours vides sont
-- présents à zéro : c'est la seule façon qu'un graphique montre un creux.
...
as $$
  ...
  select jsonb_build_object(
    ...
    'series', (
      -- Trente jours, jour par jour, zéros compris : un jour sans
      -- inscription doit apparaître comme un creux, pas disparaître.
      select jsonb_agg(jsonb_build_object(
        'jour', to_char(d.jour, 'YYYY-MM-DD'),
        'inscriptions', (select count(*) from user_accounts u where u.created_at >= d.jour and u.created_at < d.jour + interval '1 day'),
        'ventes', (select count(*) from market_transactions t where t.sold_at >= d.jour and t.sold_at < d.jour + interval '1 day'),
        'volume', coalesce((select sum(price) from market_transactions t where t.sold_at >= d.jour and t.sold_at < d.jour + interval '1 day'), 0),
        'achats', (select count(*) from payment_intents p where p.status = 'PAID' and p.settled_at >= d.jour and p.settled_at < d.jour + interval '1 day'),
        'cents', coalesce((select sum(amount_cents) from payment_intents p where p.status = 'PAID' and p.settled_at >= d.jour and p.settled_at < d.jour + interval '1 day'), 0)
      ) order by d.jour)
